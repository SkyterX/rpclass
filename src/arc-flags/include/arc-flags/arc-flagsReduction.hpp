...
namespace arcflags
{
	...
	void UpdateArcFlags(Graph& graph, ArcFlagsMap& arcflags,
		std::vector<typename ArcFlagsMap::value_type>& allArcFlags,
		ArcFlagStorage<typename ArcFlagsMap::value_type, N>& selectedArcFlags) {
		using namespace std;
		using namespace graph;
		using namespace graphUtil;
		...
		for (auto& arcFlag : selectedArcFlags.ArcFlags) {
			arcFlagMap[arcFlag] = arcFlag;
		}
	...
	void arcflags_reduce_greedy(Graph& graph, ArcFlagsMap& arcflags, double filter = 0) {
		...
		stable_sort(allArcFlags.begin(), allArcFlags.end(), arcFlagComparer);
		reverse(countedArcFlags.begin(), countedArcFlags.end());
		...
		UpdateArcFlags(graph, arcflags, allArcFlags, selectedArcFlags);
	}
	...
	void arcflags_reduce_ranked(Graph& graph, ArcFlagsMap& arcflags, double filter = 0) {
		...
		unordered_map<ArcFlagType, double> arcFlagCount;
		cout << "Counting arc-flags" << endl;
		for (const auto& v : Range(vertices(graph))) {
			for (const auto& e : Range(out_edges(v, graph))) {
				++arcFlagCount[get(arcflags, e)];
		...
		for (auto& arcFlag : Range(allArcFlags.rbegin(), allArcFlags.rend())) {
			PropagateRank<N>(arcFlag, arcFlagCount);
		}

		cout << "Sorting arc-flags" << endl;
		auto targetSize = static_cast<size_t>(ceil(allArcFlags.size() * (1.0 - filter)));
...
//		reverse(countedArcFlags.begin(), countedArcFlags.end());
		...
		cout << "Final  size" << arcFlagCount.size() << endl;
	}
}
