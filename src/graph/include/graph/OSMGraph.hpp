...
namespace graphOSM {
	...
	inline std::unique_ptr<OSMGraph> ReadGraphFromDdsg(graphIO::IReader& reader) {
		...
		std::unique_ptr<OSMGraph::Builder> builder = std::make_unique<OSMGraph::Builder>(vertexCount, linksCount*2); // DIR_BOTH edges are duplicated, so reallocating of a vector doubles the reading time
